...
def find_best_move(board, depth=7, top_n=5):
    best_move = None
    best_eval = -float('inf')
    alpha = -float('inf')
    beta = float('inf')
...
def play_chess():
    ...
    print(board)  # Display the initial board

    while not board.is_game_over():
        if board.turn:  # White's turn (Human or AI depending on setup)
            move = input("Enter your move in UCI format (e.g., e2e4): ")
            try:
                board.push_uci(move)  # Push the player's move
            except ValueError:
                print("Invalid move. Please try again.")
                continue
        else:  # Black's turn (AI)
            print("AI is thinking...")
            ai_move = predict_move(board)  # AI selects the move
            print(f"AI plays: {ai_move}")
            board.push_uci(ai_move)  # Push AI's move

        print(board)  # Display the updated board

    print("Game Over!")
    print(f"Result: {board.result()}")  # Display the result (e.g., 1-0, 0-1, 1/2-1/2)

play_chess()

def play_game2(board, depth):
    while not board.is_game_over():
        print("\nCurrent board:\n", board)
